...
export default class ExpenseForm extends React.Component {
  ...
  onAmountChange = (e) => {
    ...
    if(!amount || amount.match(/^\d{1,}(\.\d{0,2})?$/)){ // If amount is blank ("" string is falsey value, !amount === true) or amount is a match, set that value. This is handled later by our error handler inside onSubmit.
  ...
  onSubmit = (e) => {
    ...
    if(!this.state.description || !this.state.amount){
      ...
      this.props.onSubmit({ // Fired method passed down from as a prop from parent component.
        ...
        createdAt: this.state.createdAt.valueOf() // Gets milliseconds from moment object.
      }) // Passed down...
    }
  }
    ...
    return (
      <div>
        <form onSubmit={this.onSubmit}>
          ...
          {this.state.error && <p className="errorWarning">{this.state.error}</p>}
        </form>
      </div>
    );
  }
